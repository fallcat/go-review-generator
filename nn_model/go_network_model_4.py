        ...
        for i, data in enumerate(train_loader, 0):
            ...
            if i % 200 == 199:
                ...
                if average_valid_loss <  best_valid_loss:
                    ...
                    save_checkpoint('D:/GoReview' + '/model_Dot.pt', model, optimizer, best_valid_loss)
...
class MyModel(nn.Module):
    def __init__(self, dimension=64):
        ...
        self.relu = nn.ReLU()
        self.sp = nn.Softplus()
        self.fc1 = nn.Linear(364*128, 128*19)
        self.fc2 = nn.Linear(128*19, 64)
    ...
    def forward(self, x_board, x_move, text, text_len):
        ...
        x12 = torch.cat((x1, x2), dim=1).unsqueeze(2)
        x3 = x3.unsqueeze(1)
        x = torch.matmul(x12, x3)
        x = x.view(-1, 364*128)
        #x = self.bil(x, x3)
        #x = self.drop(x)
