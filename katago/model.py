...
class Model:
  ...
  #Returns the new idx, which could be the same as idx if this isn't a good training row
  def fill_row_features_optimized(self, board, pla, opp, boards, moves, move_idx, rules, bin_input_data, global_input_data, idx):
    #Currently only support v4 or v5 or v7 MODEL features (inputs version v3 and v4 and v6)
    assert(self.version == 4 or self.version == 5 or self.version == 7 or self.version == 8)
    ...
    # # print("board.board", board.board.shape)
    # # print(board.board)
    # # print('pla', pla)
    # # print('opp', opp)
    # # start = time.time()
    inner_board = np.array(board.board[:-1].reshape(bsize + 2, bsize + 1)[1:-1, 1:])
    # # print(inner_board)
    # bin_input_data[idx, inner_board.reshape(-1) == pla, 1] = 1.0
    # bin_input_data[idx, inner_board.reshape(-1) == opp, 2] = 1.0
    # loc_indices = np.array(range(1, bsize + 1)).reshape((1, -1)) \
    #               + board.dy * np.array(range(1, bsize + 1)).reshape((-1, 1))
    # inner_board_pla_or_opp = (inner_board.reshape(-1) == pla) | (inner_board.reshape(-1) == opp)
    # locs = loc_indices.reshape(-1)[inner_board_pla_or_opp]
    # libs = board.num_liberties_batch(locs)
    # libs_arr = np.zeros((bsize * bsize))
    # libs_arr[locs] = libs
    # # bin_input_data[idx, loc_indices[inner_board_pla_or_opp], 3]
    # bin_input_data[idx, libs_arr.reshape(-1) == 1, 3] = 1.0
    # bin_input_data[idx, libs_arr.reshape(-1) == 2, 4] = 1.0
    # bin_input_data[idx, libs_arr.reshape(-1) == 3, 5] = 1.0
    ...
    if rules["scoringRule"] == "SCORING_AREA" and rules["taxRule"] == "TAX_NONE":
      ...
      nonPassAliveStones = True
      unsafeBigTerritories = True
      board.calculateArea_optimized(area,nonPassAliveStones,safeBigTerritories,unsafeBigTerritories,rules["multiStoneSuicideLegal"], inner_board)
    else:
      hasAreaFeature = False
      keepTerritories = False
      keepStones = False
