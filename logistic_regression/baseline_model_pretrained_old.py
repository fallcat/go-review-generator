...
def create_board_feature_matrix(session, model, fname):
	...
	for board in data:
		...
		## print keys
		modified_board = katago.extract_features(session, model, board_arr, color)
		print(f'modified_board dict keys: {modified_board.keys()}')
		# flattened_board = board[3].flatten().tolist()
		# modified_board = [board[0],board[1],stones_dict[board[2]]]+flattened_board
		temp.append(dict(zip(range(len(modified_board['trunk'])),modified_board['trunk']))) ## (19,19,256) try flattening or sum over the last dimension
	X.fit(temp)
...
def get_examples(boards_mats, text_mats, fnames):
	...
	for boards_mat, text_mat, fname in zip(boards_mats, text_mats,fnames):
		...
		for line in choices:
			...
			cur_indices.remove(int(pos_idx))
...
if __name__ == '__main__':
	...
	with tf.Session() as session:
		...
		print('\n---TRAIN EXAMPLES---')
		train_board_feature_matrix = create_board_feature_matrix(session, model, train_boards_fname) ## 'numpy.ndarray', <class 'sklearn.feature_extraction._dict_vectorizer.DictVectorizer'>
		train_text_feature_matrix  = create_text_feature_matrix(session, model, train_text_fname) ## <class 'numpy.ndarray'>
		...
		print('\n---VALIDATION EXAMPLES---')
		val_board_feature_matrix = create_board_feature_matrix(session, model, val_boards_fname)
		val_text_feature_matrix  = create_text_feature_matrix(session, model, val_text_fname)
		X_val,  y_val = get_examples([val_board_feature_matrix], [val_text_feature_matrix], [val_choices_fname])

		print('\n---TEST EXAMPLES---')
		test_board_feature_matrix = create_board_feature_matrix(session, model, test_boards_fname)
		test_text_feature_matrix  = create_text_feature_matrix(session, model, test_text_fname)
